  ...
  try {
    ...
    const project = await db.project.findUnique({
      ...
      include: {
        ...
        competitorVideos: { orderBy: { capturedAt: "desc" } },
        interviewSession: true,
        thumbnailBriefs: { orderBy: { createdAt: "desc" } },
        productionScenes: { orderBy: { sceneNumber: "asc" } },
        trendSignals: { orderBy: { capturedAt: "desc" }, take: 30 },
      },
    });
    ...
    const decoded = {
      ...
      competitorVideos: project.competitorVideos.map((c) => ({
        id: c.id,
        ...
        titleAnalysis: jparseObj(c.titleAnalysis),
        thumbnailAnalysis: jparseObj(c.thumbnailAnalysis),
        transcriptSummary: jparseObj(c.transcriptSummary),
        commentInsights: jparseObj(c.commentInsights),
        winningPatterns: jparseArr(c.winningPatterns),
        performanceScore: c.performanceScore,
        capturedAt: c.capturedAt.toISOString(),
      })),
      interviewSession: project.interviewSession
        ? {
            id: project.interviewSession.id,
            projectId: project.interviewSession.projectId,
            status: project.interviewSession.status,
            topicsCovered: jparseArr(project.interviewSession.topicsCovered),
            questions: jparseArr(project.interviewSession.questions),
            extracted: jparseArr(project.interviewSession.extracted),
            turnCount: project.interviewSession.turnCount,
            startedAt: project.interviewSession.startedAt.toISOString(),
            updatedAt: project.interviewSession.updatedAt.toISOString(),
          }
        : null,
      thumbnailBriefs: project.thumbnailBriefs.map((b) => ({
        id: b.id,
      ...
      productionScenes: project.productionScenes.map((s) => ({
        id: s.id,
        ...
        brollSuggestions: jparseArr(s.brollSuggestions),
        motionGraphics: jparseArr(s.motionGraphics),
        editorInstructions: s.editorInstructions,
        captions: jparseArr(s.captions),
        transitions: jparseArr(s.transitions),
        assetRequirements: jparseArr(s.assetRequirements),
        retentionNotes: s.retentionNotes,
        createdAt: s.createdAt.toISOString(),
      })),
      trendSignals: project.trendSignals.map((t) => ({
        id: t.id,
